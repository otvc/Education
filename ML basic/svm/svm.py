...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = torch.exp((torch.pow(x_1, 2) @ torch.ones((x_1.shape[1], x_2.shape[0])) - 2 * x_1 @ (x_2.T) + torch.ones(x_1.shape)@(torch.pow(x_2, 2)).T)/(-2*sigma**2))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return (self.betas.T @ K.T - self.bias).view(-1)### YOUR CODE HERE
